...
KioskRunner проектируется для направления **1C HTML Shell / Self-Checkout Showcase**. v0.2 меняет deployment-канон: Apache больше не является обязательным компонентом. Основной recommended path - KioskRunner сам поднимает локальный HTTP endpoint и отдаёт текущую витрину из `current`.
...
## 7. Embedded Web Server Contract
...
## 16. Optional external publishing / reverse proxy mode
...
## 21. Формат результата и запреты для следующего этапа
...
- KioskRunner теперь объединяет updater, local storage, static serving, health/status и rollback;
- canonical URL для 1С изменён на `http://127.0.0.1:8787/kiosk/bolars/`;
- Apache/Nginx переведены в optional external publishing / reverse proxy mode;
- добавлены `webServer` config contract, health/status endpoints, static serving security и новые failure modes;
