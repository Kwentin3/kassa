...
Если `fetch`, XHR или backend collector недоступны, страница все равно показывает JSON-отчет диагностики и дает manual copy/download fallback. Это относится только к диагностическому отчету, не к runtime-доставке каталога showcase. Каталог пользователь руками не вставляет.
...
Фактический Slice 1 не загружает отдельные CSS/JS-файлы по URL. Проверки `resource.loading.css` и `resource.loading.js` используют встроенные `data:`-ресурсы и показывают только то, что среда умеет динамически подключить такой ресурс после запуска страницы. Эти проверки не доказывают работу путей, TLS, кэша, same-origin внешних файлов или политики WebView для отдельных ресурсов.
...
- загрузка CSS из встроенного `data:`-ресурса как отдельный тест;
- загрузка JavaScript из встроенного `data:`-ресурса как отдельный тест.

Важно: эти два resource-теста в Slice 1 не являются доказательством, что 1С HTML-render загрузит реальные отдельные CSS/JS-файлы по URL. Перед split-assets вариантом нужен отдельный same-origin resource smoke.
...
- `resource.loading.css/js` в Slice 1 означает проверку встроенного `data:`-ресурса, а не реальных внешних файлов.
...
- resource-тесты `resource.loading.css/js` проверяют встроенный `data:`-ресурс и не доказывают загрузку отдельных CSS/JS-файлов по URL;
...
- после аудита выполнения уточнено: `resource.loading.css/js` в Slice 1 проверяют встроенные `data:`-ресурсы, а не реальные отдельные файлы по URL.
...
- чтобы успешный resource-тест не создавал ложного вывода о готовности split-assets поставки.
