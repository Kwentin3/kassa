...
- Последняя проверка: `npm run typecheck`, `npm run test:run`, `npm run build`, local BOLARS search/phone Playwright smoke, `npm run visual:cards`, `npm run smoke:showcase-catalog` прошли.
- Тесты: 11 files / 55 tests.
...
- Latest deployed visual change: BOLARS centralized button surface refactor, commit `643c8a4`; customer-facing BOLARS buttons now have darker/contrast normal fills, stronger shadow, pointer hover lift, touch pressed depression, explicit disabled state and reduced-motion override.
...
- Latest BOLARS button surface refactor: customer-facing BOLARS buttons now use a centralized CSS button-surface contract in `src/styles/index.css` with darker/contrast normal fills, stronger shadows, pointer hover lift, touch pressed depression, explicit disabled state and reduced-motion override. Contract docs updated in `docs/product-ux/VISUAL_CONTRACTS.md`, `docs/design/VISUAL_CONTRACT_BOLARS_SELF_CHECKOUT.md`, `docs/design/BOLARS_THEME_AND_TOKENS_CONTRACT.md`; implementation/deploy report is `docs/reports/2026-05-24/BOLARS_BUTTON_SURFACE_REFACTOR.report.md`.
- Latest BOLARS search/phone input tweak: implementation commit `9d3b6b1`; cart search input now opens a UI-owned on-screen touch keyboard with RU/EN letters, digits, space, clear, backspace and close; search command still fires only after `uiConfig.searchMinLength=4`, candidate tap/outside tap hides the keyboard. Payment phone input now opens a central UI-owned numeric numpad modal, shows a fixed `+7` prefix for local Russian digits and dispatches existing `applyDiscountByPhone(phone)`. Contract docs updated in BOLARS PRD, screen composition, visual contract, acceptance checklist and runtime port notes; report: `docs/reports/2026-05-24/BOLARS_SEARCH_KEYBOARD_PHONE_NUMPAD.report.md`.
- Latest BOLARS 1C handoff tweak: concise integration guide for 1С programmers lives at `docs/integrations/BOLARS_1C_PROGRAMMER_HANDOFF.md`; debug panel links to the GitHub document as `1C handoff`. Contract audit clarified current OneC debug/smoke route, reserved ack/fail helpers, snapshot field names, search candidate echo rules and phone `+7` payload format. The handoff now includes plain-language explanations for Web/API/command/payload/queue/snapshot/adapter/debug terms; report: `docs/reports/2026-05-24/BOLARS_1C_CONTRACT_AUDIT.report.md`.
- Деплой на сервер выполнен для commit `bed18da`, контейнер `kassa-web` running; public smoke for root, BOLARS route, BOLARS debug/preview, legacy visual cards and showcase catalog passed on `https://kassa.speechbattle.com`.
