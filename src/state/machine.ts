...
export const canOpenBrandingDemo = (state: TerminalState): boolean =>
  state.name === 'idle' || state.name === 'promo_idle' || state.name === 'staff_mode' || state.name === 'branding_demo';
...
export const transition = (state: TerminalState, event: TerminalEvent): TerminalState => {
  switch (event.type) {
    ...
    case 'PAYMENT_SUCCESS':
      return { name: 'receipt_success', receiptId: event.receiptId };
    case 'PAYMENT_FAILED':
      return { name: 'payment_error', reason: event.reason, canRetry: true };
    case 'RECEIPT_FAILED':
      return { name: 'receipt_error', reason: event.reason };
    case 'REQUEST_HELP':
      return { name: 'help_requested', source: event.source };
    case 'ENTER_STAFF_MODE':
      return { name: 'staff_mode', source: event.source };
