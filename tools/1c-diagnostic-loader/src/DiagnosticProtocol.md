...
Статус: Slice 2 draft, диагностический обмен

Этот протокол нужен только для проверки периметрного диагностического обмена HTML ↔ 1С. Он не является боевым bridge-протоколом кассы и не должен вызывать РМК, чек, оплату, ККТ, фискализацию или маркировку.

Минимальная цель Slice 2: HTML отправляет `diag.ping`, 1С принимает команду и возвращает ответ обратно в HTML с тем же `requestId`.
